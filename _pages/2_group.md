---
layout: profiles-custom
permalink: /group/
title: group
#description:  <p>I am currently recruiting students at Rutgers. If you're interested in joining my group, please take a look at <a href="/_pages/prospective_students/">this page</a>.</p>
nav: true
nav_order: 3

note: <p>I am recruiting PhD students for Fall 2027. If you're interested in joining my group, please take a look at <a href="/prospective_students/">this page</a>.</p>

# Photos: put a square image in assets/img/group/ and set "image: group/<file>";
# group/placeholder.png is a neutral placeholder. "url" makes the name a link
# (homepage, GitHub or LinkedIn). A group with "compact: true" shows smaller photos.
groups:
  - name: Group
    profiles:
      - name: Zhipeng Yang
        affiliation: PhD student @ Rutgers
        image: group/zhipeng_yang.jpg
        url: https://yzp11.github.io/

      - name: Neil Gandhi
        affiliation: BS student @ Rutgers
        image: group/placeholder.png
        url: ""

      - name: Demian Samus
        affiliation: MS student @ Taras Shevchenko National University of Kyiv
        image: group/placeholder.png
        url: ""

      - name: Zihua Meng
        affiliation: MS student @ University of Pennsylvania
        image: group/placeholder.png
        url: ""

      - name: Andrew Chung
        affiliation: MS student @ Columbia University
        image: group/placeholder.png
        url: ""

      - name: Rutva Patel
        affiliation: MS student @ University of Pennsylvania
        image: group/placeholder.png
        url: ""

      - name: Abhishu Oza
        affiliation: MS student @ Rutgers
        image: group/abhishu_oza.jpg
        url: https://abhishuoza.github.io/

      - name: Timothy Hsu
        affiliation: BS student @ Rutgers (Aresty Research Assistant Program)
        image: group/placeholder.png # TODO: photo (LinkedIn photos cannot be pulled automatically)
        url: https://www.linkedin.com/in/timhsu7

      - name: Nikhil Sethuraman
        affiliation: BS student @ Rutgers (Aresty Research Assistant Program)
        image: group/nikhil_sethuraman.jpg
        url: ""

      - name: Aryaman Mishra
        affiliation: BS student @ Rutgers (Aresty Research Assistant Program)
        image: group/placeholder.png # TODO: photo (LinkedIn photos cannot be pulled automatically)
        url: https://www.linkedin.com/in/aryamanm24

      - name: Maria George
        affiliation: BS student @ Rutgers
        image: group/placeholder.png
        url: ""

      - name: Prisha Priyadarshini
        affiliation: BS student @ Rutgers
        image: group/placeholder.png
        url: ""

  - name: Alumni
    compact: true
    profiles:
      - name: Arnav Sirigere
        affiliation: BS student @ UIUC (DIMACS REU, summer 2026)
        image: group/placeholder.png
        url: ""

      - name: Anastasia Mazur
        affiliation: MS student @ Ukrainian Catholic University (RAI4Ukraine)
        image: group/placeholder.png
        url: ""

      - name: Ostap Pavlyshyn
        affiliation: BS student @ Ukrainian Catholic University (RAI4Ukraine)
        image: group/placeholder.png
        url: ""

      - name: Bohdan Turbal
        affiliation: BS student @ Taras Shevchenko National University of Kyiv (RAI4Ukraine) -> PhD student @ Princeton
        image: group/bohdan_turbal.jpeg
        url: https://ua.linkedin.com/in/bogdan-turbal-964a35222

      - name: Iryna Voitsitska
        affiliation: BS student @ Ukrainian Catholic University (RAI4Ukraine)
        image: group/iryna_voitsitska.png
        url: "" # TODO: previously pointed to Bohdan's LinkedIn by mistake

      - name: Shihan Feng
        affiliation: MS student @ Duke -> PhD student @ UNC
        image: group/shihan_feng.jpg
        url: https://shihanf0.github.io/

      - name: Ethan Hsu
        affiliation: BS student @ Duke -> Amazon
        image: group/placeholder.png
        url: https://www.linkedin.com/in/ethsu00/

      - name: Harry Chen
        affiliation: BS student @ Duke -> PhD student @ MIT
        image: group/harry_chen.jpg
        url: https://www.linkedin.com/in/harry-chen/

      - name: Dennis Tang
        affiliation: BS student @ Duke -> PhD student @ Duke
        image: group/dennis_tang.jpg
        url: ""

      - name: Zack Boner
        affiliation: PhD student @ Duke
        image: group/zack_boner.jpg
        url: https://www.linkedin.com/in/zachery-boner/

      - name: Chloe Zhu
        affiliation: BS student @ Duke -> PhD student @ Duke
        image: group/chloe_zhu.jpeg
        url: https://www.linkedin.com/in/chloe-qinyu-zhu/

      - name: Muhang (Tony) Tian
        affiliation: BS student @ Duke -> PhD student @ NYU
        image: group/tony_tian.png
        url: https://scholar.google.com/citations?user=sdzH93QAAAAJ&hl=en

      - name: Flora Shi
        affiliation: BS student @ Duke -> PhD student @ MIT
        image: group/flora_shi.jpeg
        url: https://www.linkedin.com/in/flora-shi-46976417b/
---
